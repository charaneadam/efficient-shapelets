...
class FastShapeletCandidates:
    ...
    def _get_ordered_ed_to_criteria(self, l_ts, criteria):
        ...
        """
        return sorted(
            [(i, numpy.linalg.norm(ts - criteria)) for i, ts in enumerate(l_ts)],
            key=lambda x: x[1],
        )
    ...
    def _calc_adjacent_discrepancies(self, l_ts_dists):
        ...
        """
        return [
            abs(ts_dist[1] - l_ts_dists[i + 1][1])
            for i, ts_dist in enumerate(l_ts_dists[:-1])
        ]
    ...
    def _get_splitpoints_class_to_subclasses(self, l_adj_discr):
        ...
        return [
            i for i, discr in enumerate(l_adj_discr) if discr >= std * self.std_split
        ]
    ...
    def _sort_segments_by_weight(self, l_segments):
        ...
        l_segments_weights = sorted(
            [
                (i, self._calc_weight_of_segment(t_segment[0]))
                for i, t_segment in enumerate(l_segments)
            ],
            key=lambda x: x[1],
            reverse=True,
        )
    ...
    def identify_LFDPs_of_ts(self, ts):
        ...
        if isinstance(self.n_or_threshold, int) and not 2 < self.n_or_threshold < len(
            ts
        ):
            raise ValueError(
                "Number of LFDPs must be higher than 2 and lower than the length of the time series."
            )
        if isinstance(self.n_or_threshold, float) and not self.n_or_threshold > 0:
            raise ValueError(
                "The distance threshold to the fitting line must be larger than 0."
            )
        ...
        while True:
            ...
            if (
                isinstance(self.n_or_threshold, float)
                and lfdp_dist < self.n_or_threshold
            ):
                break
    ...
    def _generate_shapelet_candidates_from_LFDPs(self, ts, l_lfdp):
        ...
        for j in range(len(l_lfdp) - 3):
            ...
            for i in range(j + 2, len(l_lfdp) - 1):
                ...
                shapelet_candidate -= numpy.mean(shapelet_candidate)
                shapelet_candidate /= numpy.std(shapelet_candidate)
...
class FastShapeletSelectionTransform:
    ...
    def fit(self, X, y):
        ...
        for label in labels:
            self.candidates[label] = fss.transform(X[y == label])
        if len(self.candidates) == 0:
            raise TransformationFailrue("No candidates found!")


def candidates_distances_labels(data, fss):
    labels = []
    dists = []
    candidates = []
    for label, cands in fss.candidates.items():
        n_cands = len(cands)
        if n_cands == 0:
            continue
        dists.append(compute_distances_shapelets_ts(data.X_train, cands))
        candidates.extend(cands)
        labels.extend([label] * n_cands)
    distances = numpy.concatenate(dists, axis=1)
    return candidates, distances, labels
...
def get_Ks(fss):
    Ks = [3, 5, 10, 20, 50, 100]
    min_n_candidates = numpy.inf
    for _, cands in fss.candidates.items():
        min_n_candidates = min(min_n_candidates, len(cands))
    return [k for k in Ks if k <= min_n_candidates]


def run_dataset(dataset_id):
    data = get_data(dataset_id)

    fss = FastShapeletSelectionTransform()
    fss.fit(data.X_train, data.y_train)
...
def _precompute_test_distances_per_method(data, candidates, method, df, max_k):
    shapelets_indices = {}
    shapelets_distances = {}
    for label in set(data.y_train):
        shapelets_indices[label] = list(
            df[df.label == label]
            .sort_values(by=method, ascending=False)
            .index[:max_k]
            .values
        )
        shapelets = [candidates[i] for i in shapelets_indices[label]]
        shapelets_distances[label] = _transform(data.X_test, shapelets)
    return shapelets_indices, shapelets_distances


def _get_transformed_data(labels, k, distances, test_indices, test_distances):
    sel = []
    for label in labels:
        sel.extend(test_indices[label][:k])
    X_tr = distances[:, sel]
    X_te = numpy.concatenate([test_distances[l][:, :k] for l in labels], axis=1)
    return X_tr, X_te


def classify(dataset_id, data, candidates, distances, Ks, labels):
    methods = ["silhouette", "fstat", "gain"]
